...
| **`DOM-Based XSS (Non persistent)`** | Ocurre completamente en el **navegador** sin intervención del servidor, cuando JavaScript manipula el **DOM** de forma insegura y ejecuta código malicioso. Menos peligroso que los anteriores |
## Ejemplos clásicos 
### Stored ? Self-Stored? DOM-Based?
Supongamos que podemos añadir un campo de texto, por ejemplo en un To-Do List, de manera que todo el código que pongamos se muestra en el navegador. Si la aplicación inserta esa entrada en un contexto ejecutable o en un sink DOM peligroso sin tratarla de forma segura, el navegador puede interpretarla como código activo
```html
<script>alert("¿XSS detectado");</script>
```
- **Stored XSS**: Se trataría de un Stored XSS si la información que introduce un usuario se guarda en la base de datos y puede ser visualizada por otros usuarios (por ejemplo, en comentarios o publicaciones compartidas).
- **Self-Stored XSS**: Ocurre cuando el payload se almacena en el backend, pero solo es visible para el propio usuario que lo introdujo. Aunque el impacto suele ser menor, sigue considerándose Stored XSS porque el código se guarda en el servidor.
- **DOM-Based XSS**: Se produce cuando el payload no se almacena en el servidor y la vulnerabilidad ocurre completamente en el navegador, normalmente al procesar datos provenientes del DOM (por ejemplo, `location`, `hash`, `localStorage` o parámetros manipulados por JavaScript) que se insertan de forma insegura en la página.
### Reflected
Es un tipo de vulnerabilidad no persistente, pero un **Reflected XSS puede ser igual de crítico que un Stored XSS**, e incluso más en ciertos escenarios (por ejemplo, robo de sesión en páginas de login, bypass MFA, phishing activo).
Una de las formas más comunes es persuadir al usuario para que abra un enlace, aunque se puede activar de más modos: 
- redirecciones automáticas
- enlaces embebidos en emails HTML
- páginas que cargan recursos automáticamente
- ataques clickjacking
- open redirects encadenados
Ejemplo clásico:
```html
https://example.com/login?msg=<script>stealCookie()</script>
```
## Herramientas de automatización
Casi cualquier escaner de vulnerabilidades Web ( [Nessus](https://www.tenable.com/products/nessus), [Burp Pro](https://portswigger.net/burp/pro), or [ZAP](https://www.zaproxy.org/)) tiene capacidad de detectar ataques XSS de cualquier tipo
### XSS Strike
Herramienta puesta en `/opt` 
```bash
...
```
### Brute XSS
Solo se pone el nombre
### XSSer
Solo se pone el nombre

## Payloads propuestos por htb
|                                                                                               |                               |
| --------------------------------------------------------------------------------------------- | ----------------------------- |
| **XSS Payloads**                                                                              |                               |
